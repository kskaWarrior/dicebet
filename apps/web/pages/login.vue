...
<template>
  <div class="card">
    <h1>{{ mode === "signin" ? t("login.signin") : t("login.signup") }}</h1>
    <form @submit.prevent="submit">
      <input v-model="email" type="email" :placeholder="t('login.email')" required autocomplete="email" />
      <input
        v-model="password"
        type="password"
      ...
      <button :disabled="busy" type="submit">
        {{ mode === "signin" ? t("login.signin") : t("login.signupBtn") }}
      </button>
    </form>
